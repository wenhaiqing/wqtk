...
<link rel="stylesheet" href="<?php  echo $_W['siteroot'];?>addons/tiger_taoke/template/mobile/user/css/face.css" />
<script src="<?php  echo $_W['siteroot'];?>addons/tiger_taoke/template/mobile/user/js/face.js"></script>
<div class="release">
...
<div class="am-modal am-modal-alert" tabindex="-1" id="fq_tips_alert">
    <div class="am-modal-dialog" style="border-radius:6px;max-width:392px;">
        ...
        <div class="am-modal-hd" style="padding-top:0;padding-bottom:.6rem;font-size:1.4rem;">
            <img src="<?php  echo $_W['siteroot'];?>addons/tiger_taoke/template/mobile/user/images/order_tips.png" style="max-width:100%;display:block;" />
        </div>
...
<?php  if($fans['follow']!=1) { ?>
<script src="<?php  echo $_W['siteroot'];?>addons/tiger_taoke/template/mobile/js/layer_mobile/layer.js"></script>
<script>
layer.open({	
	title:'-发布晒单，请先长按下图-',
    content: "<div style='width:100%;'><img src='<?php  echo $_W['account']['qrcode'];?>' width='100%'><br><font color='red'>只限会员使用本功能</font></div>",
    btn: ['返回首页', '返回微信'],
    shadeClose: false,
	closeBtn: false,
    yes: function(){
...
//                
//            }
            if (!(/^\d{16}$/.test(order)===true || /^\d{17}$/.test(order)===true)) {
                    $('#fq_alert_info').text('亲，订单号为16位或17位的数字哦！');
                    $('#fq_alert').modal({});
                    not_scroll();
                    return false;
                }
