...
<head>
    ...
    <title><?php  if($cfg['hztype']) { ?><?php  echo $cfg['hztype'];?><?php  } else { ?>积分<?php  } ?>排行榜</title>
...
<div class="ordernav">
        ...
        <a href="<?php  echo $this->createMobileUrl('ranking',array('pid'=>$pid,'dluid'=>$dluid));?>" class="cur"><span><?php  if($cfg['hztype']) { ?><?php  echo $cfg['hztype'];?><?php  } else { ?>积分<?php  } ?>排行榜</span></a>
</div>
...
<div class="allorder">
    <ul id="lists">
    <?php  if($shares) { ?>
        <?php  if(is_array($shares)) { foreach($shares as $k => $f) { ?>
          <div class="usertx"><span class="userpointer"><img src="<?php  echo preg_replace('/\/0$/', '/46', stripslashes($f['avatar']));?>" style="width: 40px; height: 40px; border-radius: 50%;">&nbsp;&nbsp;&nbsp;<?php  echo $f['nickname'];?></span><span class="usergotx" > <?php  if($cfg['paihang']==0) { ?><?php  echo $f['credit1'];?><?php  } else { ?><?php  echo $f['credit3'];?><?php  } ?> </span></div>
        <?php  } } ?>
    <?php  } else { ?>
        <div class="nocoll">
          <p></p>
