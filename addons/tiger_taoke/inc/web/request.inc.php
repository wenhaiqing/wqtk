        ...
        if ($operation == 'delete'){
            ...
            if($goods_info['type']==9){
                ...
                if($msg['message']=='success'){
                     ...
                     pdo_insert('tiger_taoke_paylog',array("uniacid"=>$_W["uniacid"],"dwnick"=>$row['realname'],"dopenid"=>$row['openid'],"dtime"=>time(),"dcredit"=>$row['cost'],"dtotal_amount"=>$row['fxprice']*100,"dmch_billno"=>$dmch_billno,"dissuccess"=>1,"dresult"=>''));  
            ...
            if($goods_info['type']==5 ||$goods_info['type']==8 ){//审核红包操作
                ...
                if($cfg['txtype']==0){
                 ...
                 if($msg['message']=='success'){
                     pdo_update($this -> table_request, $data, array('id' => $id));
                     pdo_insert('tiger_taoke_paylog',array("uniacid"=>$_W["uniacid"],"dwnick"=>$row['realname'],"dopenid"=>$row['openid'],"dtime"=>time(),"dcredit"=>$row['cost'],"dtotal_amount"=>$row['price']*100,"dmch_billno"=>$dmch_billno,"dissuccess"=>1,"dresult"=>''));  
                     message('发送成功', referer(), 'success');                  
                }else{
                      message('发送失败，错误代码：'.$msg['message'], referer(), 'success');
                }
