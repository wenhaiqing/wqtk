...
global $_W;
        ...
        $news= pdo_fetch("SELECT * FROM " . tablename($this->modulename."_fztype") . " WHERE weid = :weid" , array(':weid' => $_W['uniacid']));
        //print_r($hgoods);
        //exit;
        if ($operation == 'post'){
            $id = intval($_GPC['id']);
            ...
            if (checksubmit('submit')){
                ...
                message('更新成功！', $this -> createWebUrl('fztype', array('op' => 'display')), 'success');
            }
            ...
            $condition = '';
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->modulename."_fztype") . " WHERE weid = '{$_W['uniacid']}'  ORDER BY px ASC");
