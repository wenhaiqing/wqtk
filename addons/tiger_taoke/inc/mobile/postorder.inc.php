...
//        file_put_contents(IA_ROOT."/addons/tiger_taoke/log.txt","\n old:".json_encode($_GPC['content']),FILE_APPEND);
        ...
        file_put_contents(IA_ROOT."/addons/tiger_taoke/log--order.txt","\n 1old:".$_GPC['content'],FILE_APPEND);
...
//        file_put_contents(IA_ROOT."/addons/tiger_taoke/log--order.txt","\n 2old:".json_encode($news),FILE_APPEND);
        ...
        if(!empty($news)){
            foreach($news as $k=>$v){               
               ...
               $ord=pdo_fetch ( 'select orderid from ' . tablename ( $this->modulename . "_tkorder" ) . " where weid='{$_W['uniacid']}'  and orderid='{$v[s2]}'" );
